...
/// </remarks>
internal sealed class XHashtable<TKey, TValue>
    where TKey : notnull
{
    ...
    /// </summary>
    public delegate bool ExtractKeyDelegate(TValue value, [MaybeNullWhen(false)] out TKey key);

    /// <summary>
    /// Construct a new XHashtable with the specified starting capacity.
    /// </summary>
    public XHashtable(
        ExtractKeyDelegate extractKey,
        int capacity,
        IEqualityComparer<TKey> comparer
    )
    {
        this.comparer = comparer;
        this._state = new XHashtableState(extractKey, capacity, comparer);
    }
    ...
    {
        return this._state.TryGetValue(key, out value);
    }
    ...
    public TValue Add(TValue value)
    {
        ...
        while (true)
        {
            ...
            if (this._state.TryAdd(value, out newValue))
                return newValue;
            ...
            lock (this)
            {
                XHashtableState newState = this._state.Resize();
    ...
    private sealed class XHashtableState
    {
        ...
        public XHashtableState Resize()
        {
            ...
            for (int bucketIdx = 0; bucketIdx < this._buckets.Length; bucketIdx++)
            {
                int entryIdx = this._buckets[bucketIdx];
                ...
                if (entryIdx == EndOfList)
                {
                    ...
                    entryIdx = Interlocked.CompareExchange(ref this._buckets[bucketIdx], FullList, EndOfList);
                }
                ...
                while (entryIdx > EndOfList)
                {
                    ...
                    if(this._extractKey(this._entries[entryIdx].Value, out _))
                        newSize++;

                    if (this._entries[entryIdx].Next == EndOfList)
                    {
                        // Replace EndOfList with FullList, so that any threads still attempting to add will be forced to resize
                        entryIdx = Interlocked.CompareExchange(ref this._entries[entryIdx].Next, FullList, EndOfList);
                    }
                    else
                    {
                        // Move to next entry in the list
                        entryIdx = this._entries[entryIdx].Next;
                    }
                }
            ...
            if (newSize < this._buckets.Length / 2)
            {
            ...
            XHashtableState newHashtable = new XHashtableState(this._extractKey, newSize, this.comparer);
            ...
            for (int bucketIdx = 0; bucketIdx < this._buckets.Length; bucketIdx++)
            {
                ...
                while (entryIdx > EndOfList)
                {
                    newHashtable.TryAdd(this._entries[entryIdx].Value, out _);
        ...
        public bool TryGetValue(TKey key, [MaybeNullWhen(false)] out TValue value)
        {
            int hashCode = this.comparer.GetHashCode(key);
            int entryIndex = 0;

            // If a matching entry is found, return its value
            if (this.FindEntry(hashCode, key, ref entryIndex))
            {
                value = this._entries[entryIndex].Value;
                return true;
            }
        ...
        public bool TryAdd(TValue value, out TValue newValue)
        {
            ...
            if (!this._extractKey(value, out key))
                return true;

            // Compute hash code over entire length of key
            hashCode = this.comparer.GetHashCode(key);
            ...
            newEntry = Interlocked.Increment(ref this._numEntries);
            if (newEntry < 0 || newEntry >= this._buckets.Length)
                return false;

            this._entries[newEntry].Value = value;
            this._entries[newEntry].HashCode = hashCode;
            ...
            entryIndex = 0;
            while (!this.FindEntry(hashCode, key, ref entryIndex))
            {
                ...
                if (entryIndex == 0)
                    entryIndex = Interlocked.CompareExchange(ref this._buckets[hashCode & (this._buckets.Length - 1)], newEntry, EndOfList);
                else
                    entryIndex = Interlocked.CompareExchange(ref this._entries[entryIndex].Next, newEntry, EndOfList);
            ...
            newValue = this._entries[entryIndex].Value;
        ...
        /// </remarks>
        private bool FindEntry(int hashCode, TKey key, ref int entryIndex)
        {
            ...
            if (previousIndex == 0)
                currentIndex = this._buckets[hashCode & (this._buckets.Length - 1)];
            else
            ...
            while (currentIndex > EndOfList)
            {
                ...
                if (this._entries[currentIndex].HashCode == hashCode)
                {
                    ...
                    if (!this._extractKey(this._entries[currentIndex].Value, out var keyCompare))
                    {
                        if (this._entries[currentIndex].Next > EndOfList)
                        {
                            // PUBLISH (buckets slot or entries slot)
                            // Entry is invalid, so modify previous entry to point to its next entry
                            this._entries[currentIndex].Value = default(TValue)!;
                            currentIndex = this._entries[currentIndex].Next;

                            if (previousIndex == 0)
                                this._buckets[hashCode & (this._buckets.Length - 1)] = currentIndex;
                            else
                                this._entries[previousIndex].Next = currentIndex;
                    ...
                    else
                    {
                        ...
                        if(this.comparer.Equals(key, keyCompare))
                        {
